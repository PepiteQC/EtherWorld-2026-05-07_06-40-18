...
import type { ServerHealth } from "../../lib/api/client";
...
export default function HUD({
  ...
  serverHealth = null, serverStatus = "checking",
}: HUDProps) {
  ...
  const serverOnline = serverStatus === "online" && Boolean(serverHealth?.ok);
  const firebaseOnline = Boolean(serverHealth?.firebaseAdmin);
  ...
  return (
    <>
      ...
      {/* SERVER STATUS */}
      <div style={{
        position: "absolute", top: 92, right: 20,
        background: "rgba(0,8,18,0.82)",
        border: `1px solid ${serverOnline ? "rgba(70,220,120,0.5)" : "rgba(220,80,80,0.45)"}`,
        borderRadius: 4,
        padding: "8px 12px",
        ...
        <div style={{
          fontSize: 9,
          color: serverOnline ? "#60e880" : "#ff7070",
          letterSpacing: 1.5,
          marginBottom: 3,
        }}>
        ...
        <div style={{
          fontSize: 9,
          color: firebaseOnline ? "#60e880" : "#d0a040",
          letterSpacing: 1.5,
        }}>
          {firebaseOnline ? "● FIREBASE ADMIN ONLINE" : "● FIREBASE ADMIN WAIT"}
        </div>
      </div>
